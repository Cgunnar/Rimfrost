...
namespace Rimfrost
{
	...
	{
		m_node = new NodeHandle(handle);
		m_selectTranslate = selectTranslate;
		m_selectRotate = selectRotate;
		m_selectScale = selectScale;
	}

	NodeEditGUI& NodeEditGUI::operator=(const NodeEditGUI& other)
	{
		if (m_node) delete m_node;
		m_node = new NodeHandle(*other.m_node);
		m_selectTranslate = other.m_selectTranslate;
		m_selectRotate = other.m_selectRotate;
		m_selectScale = other.m_selectScale;
		return *this;
	}

	NodeEditGUI::~NodeEditGUI()
	{
		if(m_node) delete m_node;
		m_node = nullptr;
	}
	void NodeEditGUI::view()
	{
		...
		ImGui::Text("id: %u", m_node->get().m_ID);
		...
		auto fl3 = m_node->get().localMatrix.getTranslation();
		float pos[3] = { fl3.x, fl3.y, fl3.z };
		if (ImGui::SliderFloat3("position", (float*)&pos, -10, 10))
		{
			m_node->get().localMatrix.setTranslation(pos[0], pos[1], pos[2]);
		}
